...
The setup is similar to the 1-snake generalizable setup (see below). I chose NFSP as a baseline to see what a Nash equlibrium would look like and build from there. From the results, it is clear that one snake dies quickly and the other grows long but eventually doesn't know how to avoid itself. This seems to be more or less what happens each time, but it isn't the same snake dying since the reward plot shows that during some games each snake has high reward (long life). Furthermore, on average, their lives seem to be 30 steps or so, per the steps taken plot. However, when one snake dies too quickly, this boils it down to the one snake problem; hence, it's interesting that same structure worked in the 1-snake game but seems to fail in the 2-snake game. Perhaps it's related to the size of the board. 
