...
<img src="https://github.com/clvoloshin/RL_Algos/blob/master/research/slitherin/media/reward_93000_2_snake.png" width="250" height="150">,<img src="https://github.com/clvoloshin/RL_Algos/blob/master/research/slitherin/media/steps_taken_93000_2_snake.png" width="250" height="150">
![Gif 2 snake](./media/two_snake_93000_epoch.gif)

The setup is similar to the 1-snake generalizable setup (see below) except the board is bigger to accomodate two snakes. I chose NFSP as a baseline to see what a Nash equlibrium would look like and build from there. From the results, it is clear that one snake dies quickly and the other grows long but eventually doesn't know how to avoid itself. This seems to be more or less what happens each time, but it isn't the same snake dying since the reward plot shows that during some games each snake has high reward and, on average, their lives seem to be 30 steps or so (per the steps taken plot). However, when one snake dies too quickly, this boils it down to the one snake problem; hence, it's interesting that same structure worked in the 1-snake game but seems to fail in the 2-snake game. Perhaps it's related to the size of the board--it may be worth to go back to the 1-snake game with a bigger board to see if the networks' filters are too small in the early layers. Another thing worth noting is that once a snake starts falling behind by dying very early, it is difficult for it to catch up (learning-wise) because the number of total frames seen falls behind significantly.
